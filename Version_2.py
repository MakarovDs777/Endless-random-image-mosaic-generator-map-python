...
def load_images():
    ...
    if file_paths:
        # Не очищаем image_data перед добавлением, чтобы сохранить все загруженные изображения
        for fp in file_paths:
            img = np.array(Image.open(fp).resize((canvas_width, canvas_height)).convert("RGB"))
            image_data.append(img)
            image_listbox.insert(tk.END, os.path.basename(fp))  # Добавляем только имя файла

# Функции генерации мозаики
...
# Поле ввода для задержки авторандома
